# -*- coding: utf-8 -*-
"""
部署入口：UGC 审核 + Agent 工作单元管理（Streamlit）
...
def page_audit() -> None:
    st.title("UGC 内容审核")
    st.caption("基于当前可运行的 audit_system 编排：意图分析 → 工作单元 → 验证器 → 仲裁")
    ...
    if st.session_state.last_audit_result:
        ...
        if isinstance(d, dict):
            ...
            st.write("**最终裁决**", d.get("final_decisions"))
            st.write("**mermaid_pipeline**", (d.get("mermaid_pipeline") or "")[:120], "…")

    st.info("左侧可切换到 **Agent 管理** 维护工作单元配置，无需手改 YAML。")


def page_agents() -> None:
    st.title("Agent 工作单元管理")
    st.caption("工作单元配置写入项目根目录的 work_units_config.yaml，提示词文件保存在 audit_system/prompts/。")
    ...
    with ec2:
        ...
        if up is not None and st.button("确认导入并覆盖", type="primary"):
            try:
