...
from .workflows.ugc_moderation.workflow import build_ugc_moderation_graph, run_ugc_moderation
...
def get_workflow_graph(workflow_id: str) -> WorkflowGraph:
    ...
    if workflow.type == "ugc_moderation":
        return build_ugc_moderation_graph(workflow, agents)
...
def _dispatch_run(
    ...
    if workflow.type == "ugc_moderation":
        return run_ugc_moderation(store, workflow, user_input, history=history, on_event=on_event)
    raise HTTPException(status_code=400, detail=f"Unsupported workflow type: {workflow.type}")
