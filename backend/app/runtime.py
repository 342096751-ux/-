...
class LLMGateway:
    ...
    def run_agent(
        ...
        try:
            ...
            if not str(result or "").strip():
                return self._build_agent_fallback_text(agent, "empty_output")
            return result
        except Exception as error:  # noqa: BLE001
            return self._build_agent_fallback_text(agent, str(error))

    def _build_agent_fallback_text(self, agent: AgentDefinition, error_message: str) -> str:
        message = str(error_message or "unknown_error").strip() or "unknown_error"
        return (
            "我当前没办法回答、分析这个问题。\n"
            f"我现在出现异常，异常信息: {message}\n"
            f"(agent: {agent.name})"
        )
    ...
    def _run_agent_with_tools(
        ...
        def emit_llm_trace(stage: str, **meta: Any) -> None:
        ...
        if not filtered_skills:
            emit_llm_trace(
                "llm_started",
                prompt_preview=(str(user_input or "").strip()[:240]),
                has_tools=False,
            )
            response = self.client.chat.completions.create(
            ...
            return self._enforce_prompt_contract(
                agent=agent,
                system_prompt=system_prompt,
                user_input=user_input,
                draft=content,
        ...
        for _ in range(4):
            emit_llm_trace(
                "llm_started",
                prompt_preview=(str(messages[-1].get("content") if messages else user_input)[:240]),
            ...
            emit_llm_trace(
                "llm_finished",
                output_preview=str(message.content or "")[:300],
                has_tools=bool(tools_payload),
    ...
    def _should_enforce_prompt_contract(self, system_prompt: str) -> bool:
        text = str(system_prompt or "").lower()
        # As long as agent has a system prompt, enforce prompt contract by default.
        if text.strip():
            return True
        markers = [
            "严格",
    ...
    def _looks_like_json_contract(self, system_prompt: str) -> bool:
        text = str(system_prompt or "").lower()
        return any(marker in text for marker in ["json", "schema", "输出格式", "仅输出json", "only output json"])
    ...
    def _repair_output_to_prompt(
        ...
        *,
        ...
        repair_prompt = (
            "Rewrite the draft so it strictly follows the system prompt requirements.\n"
            "Do not add explanations, only return the final compliant output.\n\n"
            f"System prompt:\n{system_prompt}\n\n"
            f"User input:\n{user_input}\n\n"
            f"Draft output:\n{draft}"
        )
    ...
    def _enforce_prompt_contract(
        ...
        *,
        ...
        output = str(draft or "").strip()
        if not output or not self._should_enforce_prompt_contract(system_prompt):
            return output

        if self._looks_like_json_contract(system_prompt):
            latest = output
            for _ in range(3):
                normalized = self._normalize_json_text(latest)
                try:
                    json.loads(normalized)
                    return normalized
                except Exception:  # noqa: BLE001
                    try:
                        latest = self._repair_output_to_prompt(
                            agent=agent,
                            system_prompt=system_prompt,
                            user_input=user_input,
                            draft=latest,
        ...
        try:
            repaired = self._repair_output_to_prompt(
                agent=agent,
                system_prompt=system_prompt,
                user_input=user_input,
                draft=output,
