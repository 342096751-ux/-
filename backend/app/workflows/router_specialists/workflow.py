            ...
            if stage == "llm_started":
                push(
                    ...
                    event(
                        "state_updated",
                        "Model Input",
                        f"{agent.name} is preparing model response.",
                        node_id=meta.get("agent_id") or agent.id,
                        agent_id=agent.id,
                        prompt_preview=str(meta.get("prompt_preview") or "")[:220],
                        has_tools=bool(meta.get("has_tools")),
                    ),
                )
                return

            if stage == "llm_finished":
                push(
                    trace,
                    event(
                        "message_generated",
                        "Model Output",
                        f"{agent.name} produced model output.",
                        node_id=meta.get("agent_id") or agent.id,
                        agent_id=agent.id,
                        preview=str(meta.get("output_preview") or "")[:220],
