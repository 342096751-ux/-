...
import { Bot, BrainCircuit, Download, GitBranch, Sparkles, Terminal } from "lucide-vue-next";
...
function stepFromEvent(event, idPrefix) {
  ...
  return {
    ...
    modelPreview: String(payload?.preview || payload?.prompt_preview || "").trim(),
  };
}
...
function downloadTraceCsv() {
  ...
  rows.forEach((item, idx) => {
    ...
    lines.push([
      ...
      csvEscape(payload?.node_id || ""),
      csvEscape(payload?.next_node_id || ""),
      csvEscape(payload?.agent_name || ""),
      csvEscape(payload?.tool_name || ""),
      csvEscape(payload?.preview || ""),
      csvEscape(payload?.prompt_preview || ""),
      csvEscape(payload?.thinking || payload?._thinking || ""),
      csvEscape(typeof payload?.context === "string" ? payload.context : JSON.stringify(payload?.context || payload?._input || {})),
      csvEscape(payload?.raw_output || payload?._rawOutput || ""),
    ].join(","));
  });
  ...
  a.href = url;
  a.download = `trace-dialogue-${Date.now()}.csv`;
  a.click();
  URL.revokeObjectURL(url);
}

function openConversation(event) {
  modalEntry.value = event;
  ...
  <section class="glass-panel trace-shell">
    <header class="run-panel-header">
      ...
      <div class="trace-toolbar">
        <button
          type="button"
          class="ghost-button small-ghost"
          :disabled="!traceItems.length"
          @click="downloadTraceCsv"
        >
          <Download :size="14" />
          下载对话表
        </button>
            ...
            <div class="trace-group-flow">
              ...
              >
                ...
                <div class="trace-flow-body">
                  ...
                  <p v-if="step.modelPreview" class="trace-flow-detail">
                    {{ step.rawType === "state_updated" ? "输入片段" : "输出片段" }}：{{ step.modelPreview }}
                  </p>
          ...
          <div class="trace-body">
            ...
            >
              ...
              <template v-if="isExpanded(item.event, item.index)">
                ...
                <button class="ghost-button small-ghost" @click="openConversation(item.event)">查看完整上下文</button>
                <div
